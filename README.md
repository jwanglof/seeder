...
- The column is **integer-typed and has a `DEFAULT`**. V0.1 does not
  ...
  only true `nextval(...)` columns are skipped is planned for V0.2.)
...
- **Self-FK** (e.g., `employees.manager_id REFERENCES employees(id)`):
  V0.1 builds the whole table in one batch, so the parent pool is empty
  ...
  aborts. (Picking from already-generated rows is planned for V0.2.)
...
statistics sampling, raw `DEFAULT` parsing — is planned for V0.2.
